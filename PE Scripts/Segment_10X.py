...
if __name__ == '__main__':
    ...
    path_name = 'genotyping/cycle_12'
    img = isf.InSitu.Import_ND2_by_Tile_and_Well(n_tile, n_well, path_name)

    # DAPI channel
    nucs_ch = -1
    # C Nucleotide channel
    cells_ch = -2

    cell_size = 30
    nuc_size = 20

    nucs = isf.Segment.Segment_Nuclei(img[nucs_ch], nuc_diameter=nuc_size)
    cells = isf.Segment.Segment_Cells(img[cells_ch], NUC=img[nucs_ch], cell_diameter=cell_size)
